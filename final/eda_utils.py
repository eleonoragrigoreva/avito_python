...
def count_missing(data):
    """
    Считает, сколько пропусков в каждом поле и какую долю они составляют.
    ...
    Returns:
        ...
        missing_ratio: словарь {название поля: доля пропусков, округлённая до 4 знаков}
    """
...
def get_min_max_id_year(data):
    """
    Находит минимальный и максимальный id и к каким годам вручения они относятся.
...
def find_duplicate_ids(data):
    """
    Проверяет, есть ли повторяющиеся id среди лауреатов.
...
def find_id_gaps(data):
    """
    Ищет пары id, между которыми есть разрыв больше 1.
    
    Args:
        data: список словарей с лауреатами
        
    Returns:
        список кортежей (id1, id2), где id2 - следующий за id1, а разрыв больше 1
    """
